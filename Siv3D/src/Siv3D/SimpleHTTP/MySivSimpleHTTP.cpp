...
namespace s3d
{
	...
	{
		HTTPResponse Post(const URLView url, const HashTable<String, String>& headers, String name, const void* src, const size_t size, const FilePathView filePath)
		{
			...
			if (auto result = Post(url, headers, name, src, size, static_cast<IWriter&>(writer)))
			{
				return result;
			}
		...
		HTTPResponse Post(const URLView url, const HashTable<String, String>& headers, String name, const void* src, const size_t size, MemoryWriter& writer)
		{
			SIV3D_ENGINE(Network)->init();
		...
		HTTPResponse Post(const URLView url, const HashTable<String, String>& headers, String name, const void* src, const size_t size, IWriter& writer)
		{
			SIV3D_ENGINE(Network)->init();
			...
			::curl_mime_name(part, name.narrow().c_str());
			::curl_mime_filedata(part, (char*)src);
